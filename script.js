...
// Données du compteur
let remainingGenerations = MAX_GENERATIONS;
let expiryTime = Date.now() + COUNTER_DURATION;
    ...
    if (stored) {
        try {
            ...
            if (data.expiryTime > Date.now()) {
                ...
                expiryTime = Date.now() + COUNTER_DURATION;
            }
        } catch (e) {
            resetCounter();
        }
...
function saveCounter() {
    localStorage.setItem('devisCounter', JSON.stringify({
...
function updateCounterDisplay() {
    ...
    if (timerEl && remainingEl) {
        ...
        timerEl.textContent = `Temps restant : ${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
        remainingEl.textContent = `Générations restantes : ${remainingGenerations}`;
    }
    // Désactiver le bouton si plus de générations
    const pdfBtn = document.getElementById('generate-pdf');
    if (pdfBtn) {
        pdfBtn.disabled = remainingGenerations <= 0;
        pdfBtn.style.opacity = remainingGenerations <= 0 ? 0.5 : 1;
        pdfBtn.title = remainingGenerations <= 0 ? "Vous avez atteint la limite de 3 générations. Revenez plus tard." : "";
    }
}
...
function loadExample(metier) {
    ...
    const exemple = EXEMPLES_METIERS[metier];
    if (!exemple) return;

    document.getElementById('invoice-title-input').value = exemple.titre;
...
async function generatePDF() {
    ...
    if (remainingGenerations <= 0) {
        alert("Vous avez atteint la limite de 3 générations. Veuillez patienter jusqu'à la réinitialisation du compteur (6h).");
        return;
    }
    ...
    try {
        ...
        // Décrémenter le compteur après génération réussie
        decrementCounter();
        saveToHistory();
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    initCounter(); // Initialiser le compteur
    initResizer();
    loadDefaultSettings();
